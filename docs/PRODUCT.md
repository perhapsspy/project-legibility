...
| 선택형 보조 | `codex-project-director`, `codex-token-discipline`, `project-context-migration` | `codex-project-director`는 사용자가 `$codex-project-director`로 명시 호출한 세션에서 여러 Codex 작업을 지휘하고, drift 신호가 있을 때 독립적인 읽기 전용 궤적 검토를 사건 기반으로 사용해 작업자의 진행이 승인된 결과에 계속 비례하고 직접적인지 감독합니다. `codex-token-discipline`은 넓거나 예측하기 어려운 출력, 브라우저 루프, subagent, 반복 compaction처럼 명확한 초과 비용 위험이 있을 때 자동 참여합니다. 기존 작업 맥락 이관은 `project-context-migration`이 맡습니다. |
