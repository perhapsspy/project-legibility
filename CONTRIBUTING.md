...
새 스킬은 별도 실험 상태를 만들지 않고 기존 릴리스 경로로 편입합니다. 편입 전에는 정본 owner가 다음 근거를 제시하고 Project Legibility maintainer가 제품 구성과 호출 경계를 승인합니다.

- 기존 구성에서 채우는 역할 공백과 인접 스킬이 선택되지 않는 경계
- 정본 자체 검증과 대표 행동·호출 사례
- 배포 뒤 실제 사용에서 확인할 호출 적합성이나 고유 효용

편입 뒤에는 의미 있는 실제 사례만 해당 task 기록에 `관찰 버전 또는 정본 커밋`, `상황`, `예상 선택·효용`, `실제 결과`, `문제`, `현재 처리`로 남깁니다. 첫 중대한 실패, 반복되는 호출 경계 혼동, 다음 관련 수정이나 구성 변경이 검토 시점입니다. 정기 점수표나 사용량 기준은 두지 않습니다.

내용·trigger·자체 test의 개선은 정본 owner가 수행합니다. 호환되는 교정은 patch, 역할이나 trigger 의미의 변경은 제품 계약을 먼저 갱신하는 minor입니다. 별도 승격 절차 없이 문제가 없으면 유지합니다. 해결되지 않은 중대한 실패, 반복되는 오호출이나 고유 효용을 보여주지 못하는 중복이 확인되면 minor에서 제거하며, 단순한 사용량 부족만으로 제거하지 않습니다.

즉시 되돌려야 할 문제는 publisher catalog를 마지막 검증 릴리스로 돌리는 기존 rollback 절차를 사용합니다.
