...
- `purpose-first-design`이 자신의 제안이나 검토 통과를 확정 결정의 근거로 삼지 않도록 했습니다. 방향에 딸린 필수 구성까지 포함해 더 단순한 안과 비교하며, 기존 확정 의미와 안전·운영 경계는 유지합니다.
- `source-owner-audit`이 현재 owner 문서의 채택된 요구와 후보, 미확인 사실과 미결 선택을 구분하도록 했습니다. 연결된 계약 상세를 비교하며, 파생 문서의 생략만으로 계약 공백을 만들지 않습니다. 읽기 전용 범위는 유지합니다.
- 이전 태그 이후 main에 반영된 `codex-token-discipline` 사용량 집계 교정을 포함합니다. 이벤트 시각 구간과 모델·effort 귀속을 사용하고, 누락된 시각·모델·fork 경계는 추정하지 않고 미확인으로 보고합니다.
...
- `codex-token-discipline`이 출력이 작아도 비용이 큰 실행·재시도와 진행 관측에 자동 참여합니다. 이전 결과에 따라 제한된 다음 실행·관측을 선택하며, 일상적인 테스트 실행 자체는 참여 조건에서 제외합니다.
- 알려진 시간 민감한 선택을 바꿀 수 있는 기존 진행 정보를 추가 대기 전에 확인하도록 했습니다. 유효한 검증 근거의 재사용과 필요한 검증은 유지합니다.

## [0.13.1] - 2026-09-05

### Fixed

- `project-context`가 빈 `DECISIONS.md`를 아직 기록할 결정이 없는 정상 상태로 인정합니다. 빈 로그의 조회·검사와 첫 결정 추가를 지원하며, 불완전한 결정 기록은 계속 검사합니다.
- runtime-shape checker에 `--task-root`를 추가해 일상적인 완료·인계 시 현재 작업만 검사하도록 했습니다. 옵션을 생략하면 기존 저장소 전체 검사를 수행합니다.
