...
Use the bundled log helper and runtime-shape checker; their scripts own exact mechanics. For routine completion or handoff, run the skill-relative `scripts/check_runtime_shape.py --task-root docs/tasks/yyyy/mm-dd/<task-slug>`; the task path is relative to the detected repo or explicit `--repo-root`. Omit `--task-root` for a repository-wide audit or migration check. The checker covers required shape, latest log blocks, portable paths, and secret-like markers, not ownership, semantic quality, full history, or migration correctness. When a task is maintained, finish only when its brief can reopen the work and current context has one clear owner.
