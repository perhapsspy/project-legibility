...
이전 결과를 바탕으로 다음 제한된 실행 또는 관측을 선택한다. 관련 조건의 변화나 같은 조건을 반복할 구체적 근거를 사용한다. 정상 진행의 예산 소진, 아직 충족하지 못한 필수 검증, 구체적인 일시적·비결정적 실패 가설은 선택의 근거가 될 수 있다. 결정적 실패로 끝난 검사는 같은 검사를 반복하기 전에 원인을 해결한다.

이미 제공되는 진행 정보가 알려진 시간 민감한 선택을 바꿀 수 있다면 추가 대기 전에 그 정보를 확인한다. 아직 유효한 증거는 재사용하며, 이 판단을 위한 별도 보고서나 로그는 만들지 않는다.

지원되는 범위에서는 기존 도구나 현재 실행 담당자로 일상적인 상태 수집과 비교를 수행한다. 필요한 진행 보고는 유지하되, 업데이트가 실행 소유권을 parent에게 넘기지는 않는다. 완료, 해소되지 않은 blocker, 범위·acceptance·authority·risk 판단이 필요한 변경을 드러낸다. 독립적으로 유용한 책임이 있을 때만 별도 observer를 추가한다.
